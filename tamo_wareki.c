/*
 *  tamo_wareki.c
 *          by H.Tsujimura  tsupo@na.rim.or.jp
 *                  17 February 1991
 *  Copyright (c) 1991-1995,1996,1999,2003,2007  Hiroshi Tsujimura
 *  All Rights Reserved.
 *
 * History :
 * $Log: tamo_wareki.c $
 * Revision 1.76  2007/02/06  11:00:03  tsujimura543
 * copyright •\‹L‚πƒAƒbƒvƒf[ƒg
 *
 * Revision 1.75  2003/11/11  07:02:19  tsujimura543
 * K&R •\‹L ¨ ANSI C •\‹L ‚Ι®‘SΪs
 *
 * Revision 1.74  2003/03/20  09:18:02  tsujimura543
 * eν—ο’‚Μ•\¦‚π’Η‰Α
 *
 * Revision 1.73  2003/03/19  05:01:53  tsujimura543
 * ±xΦA‚Μo—Ν“ΰ—eC³I—Ή
 *
 * Revision 1.72  2003/03/19  04:46:08  tsujimura543
 * ±x‚Μo—Ν•ϋ®‚π‘Ό‚Μ—ο‚Ζ“―‚¶‚Ι‚·‚ι
 *
 * Revision 1.71  2003/03/19  03:05:48  tsujimura543
 * ‹—οΦW‚Μo—Ν•ϋ–@‚π‘Ό‚Μ—ο‚Ζ“―‚¶•ϋ®‚Ι‚·‚ι
 *
 * Revision 1.70  2003/03/18  10:20:50  tsujimura543
 * o—Ν‹‰Κ‚ª‚«‚κ‚Ά‚Ι‚Θ‚ι‚ζ‚¤‚Ι’²®
 *
 * Revision 1.69  2003/03/17  05:44:54  tsujimura543
 * HTMLo—Ν“ΰ—e‚π®—
 *
 * Revision 1.68  2003/03/11  02:07:11  tsujimura543
 * for Tsuporone's Activity Memo
 *
 * Revision 1.67  2003/03/06  10:27:39  tsujimura543
 * ‰A“ΩJn“ϊ‚Μ‹γ―‚ª1‚Β‚Έ‚κ‚ι•s‹ο‡‚πC³
 *
 * Revision 1.66  2003/03/06  08:50:26  tsujimura543
 * ‹γ―‚ΜZo•ϋ–@‚π•ΟX
 *
 * Revision 1.65  2003/02/27  10:07:05  tsujimura543
 * ‘ε‰»Θ‘O‚ΝA“Vc‚Μ–Ό‘O‚Ε”N‘γ‚π•\‚·‚ζ‚¤‚Ι‚µ‚½
 *
 * Revision 1.64  2003/02/27  08:41:01  tsujimura543
 * ‘ε‰»Θ~‚Μ³†‚Ι‘Ξ‰
 *
 * Revision 1.63  2003/02/26  09:28:53  tsujimura543
 * cδΘ‘O‚Μ³†‚Ι‘Ξ‰(‚Ζ‚θ‚ ‚¦‚ΈAc’·‚ά‚Ε‘Ξ‰)
 *
 * Revision 1.62  2003/02/19  08:50:49  tsujimura543
 * ƒ†ƒƒEƒX“ϊ‚ª•‰‚Μ’l‚Μ‚Ζ‚«‚Ι—j“ϊζ“ΎΦ”‚ªλ“®μ‚·‚ι•s‹ο‡‚πC³
 *
 * Revision 1.61  2003/02/07  04:46:41  tsujimura543
 * a—ο‚Μ–Ό‚π•Ή‹L‚·‚ι‚ζ‚¤‚Ι‚µ‚½
 *
 * Revision 1.60  2001/02/20  12:23:36  tsujimura543
 * ‘Ό‚Μ”h¶ƒo[ƒWƒ‡ƒ“‚Ζ“‡‚·‚ι‚½‚ίA‚Ά‚Α‚½‚ρ revision ‚πΕ’θ
 *
 * Revision 1.51  1999/07/06  18:48:12  tsujimura543
 * J”­Β‹«‚π Win32 ‚ΙΪ‚·
 * -- HTML o—Ν‘Ξ‰μ‹Ζ‚β“®μm”F‚Ν΅γ‚ΰUNIX (peach.na.rim.or.jp) γ‚Εs‚Θ‚¤
 *
 * Revision 1.50  1996/08/19  02:10:16  tsujimura
 * Withdrawal from `shizuka' (UNIXγ‚Ε‚ΜJ”­I—Ή”Ε)
 *
 * Revision 1.6  95/12/16 05:55:16  tsujimura
 * ‚ΰ‚Ν‚β•s—v‚Ζ‚Θ‚Α‚½u’ªv‚Μ–ΌΜ‚Μƒe[ƒuƒ‹‚πν
 * 
 * Revision 1.5  92/03/29 16:24:14  tsujimura
 * test release
 * 
 * Revision 1.4  91/03/17 16:16:28  tsujimura
 * test version
 * 
 * Revision 1.3  91/03/17 16:16:28  tsujimura
 * (1) changed functions for calculations of old-japanese-calendar
 * (2) added some ETO holidays
 * 
 * Revision 1.2  91/03/02 05:29:46  tsujimura
 * added many features
 * 
 * Revision 1.1  91/02/17 10:58:18  tsujimura
 * Initial revision
 * 
 */

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "tamo.h"

#ifndef	lint
static char	*rcs_id =
	"$Header: C:/user/local/src/tamo/RCS/tamo_wareki.c 1.76 2007/02/06 11:00:03 tsujimura543 Exp tsujimura543 $";
#endif

#if 0
static char *gengo[]         = { "•¶‹v","³΅","cδ","–Ύ΅","‘ε³","Ίa","•½¬",0 };
static int  gengo_start_yy[] = { 1861,  1864,  1865,  1868,  1912,  1926,  1989 };
static int  gengo_start_mm[] = {  2,     2,     4,     9,     7,    12,     1 };
static int  gengo_start_dd[] = { 19,    20,     7,     8,    30,    25,     8 };
#else
static struct GENGO_TBL {
    char    *name;      /* –ΌΜ    */
    int     start_yy;   /* Jn ”N */
    int     start_mm;   /*       */
    int     start_dd;   /*      “ϊ */
}   gengo_tbl[] =   {
    { "Ό—ο",      1,  1,  1 },

    { "(‰_)",  270,  1,  1 },
    { "(m“Ώ)",  313,  1,  3 },
    { "(—’†)",  400,  2,  1 },
    { "(”½³)",  406,  1,  2 },
    { "(ς‹±)",  412, 12,  1 },
    { "(ΐN)",  454,  1,  1 },
    { "(—Y—ª)",  457,  1,  1 },
    { "(΄”J)",  480,  1, 15 },
    { "(°@)",  485,  1,  1 },
    { "(m«)",  488,  1,  5 },
    { "(•—σ)",  499,  1,  1 },
    { "(p‘Μ)",  507,  2,  4 },
    { "(ΐΥ)",  534,  1,  1 },
    { "(‹X‰»)",  536,  1,  1 },
    { "(‹Τ–Ύ)",  540,  1,  1 },
    { "(•q’B)",  572,  4,  3 },
    { "(—p–Ύ)",  586,  1,  1 },
    { "(’s)",  588,  1,  1 },
    { "(„Γ)",  593,  1,  1 },
    { "(®–Ύ)",  629,  1,  4 },
    { "(c‹Ι)",  642,  1, 15 },

    { "‘ε‰»",    645,  6, 19 },
    { "”’θ³",    650,  2, 15 }, /* F“Ώ“Vc•φδγg‚ν‚Θ‚­‚Θ‚Α‚½ */
    { "(Δ–Ύ)",  655,  1,  3 },
    { "(“V’q)",  661,  7, 24 },
    { "(“V•)",  672,  1,  1 },
    { "ι’Ή",    686,  7, 20 }, /* “V•“Vc•φδγg‚ν‚Θ‚­‚Θ‚Α‚½ */
    { "(“)",  687,  1,  1 },
    { "(•¶•)",  697,  8,  1 },
    { "‘ε•σ",    701,  3, 21 },
    { "c‰_",    704,  5, 10 },
    { "a“Ί",    708,  1, 11 },
    { "—μ‹T",    715,  9,  2 },
    { "—{V",    717, 11, 17 },
    { "_‹T",    724,  2,  4 },
    { "“V•½",    729,  8,  5 },
    { "“V•½΄•σ",749,  4, 14 },
    { "“V•½•σ",749,  7,  2 },
    { "“V•½•σ",757,  8, 18 },
    { "“V•½_μ",765,  1,  7 },
    { "_μi‰_",767,  8, 16 },
    { "•σ‹T",    770, 10,  1 },
    { "“V‰",    781,  1,  1 },
    { "‰„—ο",    782,  8, 19 },
    { "‘ε“―",    806,  5, 18 },
    { "Om",    810,  9, 19 },
    { "“V’·",    824,  1,  5 },
    { "³a",    834,  1,  3 },
    { "‰ΓΛ",    848,  6, 13 },
    { "mυ",    851,  4, 28 },
    { "Δt",    854, 11, 30 },
    { "“Vΐ",    857,  2, 21 },
    { "’εΟ",    859,  4, 15 },
    { "³c",    877,  4, 16 },
    { "ma",    885,  2, 21 },
    { "°•½",    889,  4, 27 },
    { "Ή‘Χ",    898,  4, 26 },
    { "‰„μ",    901,  7, 15 },
    { "‰„’·",    923,  4, 11 /* ‰[4 */ },
    { "³•½",    931,  4, 26 },
    { "“Vc",    938,  5, 22 },
    { "“V—ο",    947,  4, 22 },
    { "“V“Ώ",    957, 10, 27 },
    { "‰a",    961,  2, 16 },
    { "N•Ϋ",    964,  7, 10 },
    { "ΐa",    968,  8, 15 },
    { "“V\",    970,  3, 25 },
    { "“V‰„",    973, 12, 20 },
    { "’ε³",    976,  7, 13 },
    { "“V³",    978, 11, 29 },
    { "‰iΟ",    983,  4, 15 },
    { "°a",    985,  4, 27 },
    { "‰i‰„",    987,  4,  5 },
    { "‰iβN",    989,  8,  8 },
    { "³—ο",    990, 11,  7 },
    { "’·“Ώ",    995,  2, 22 },
    { "’·•Ϋ",    999,  1, 13 },
    { "°O",   1004,  7, 20 },
    { "’·a",   1012, 12, 25 },
    { "°m",   1017,  4, 23 },
    { "΅ΐ",   1021,  2,  2 },
    { "–υ",   1024,  7, 13 },
    { "’·³",   1028,  7, 25 },
    { "’·—ο",   1037,  4, 21 },
    { "’·‹v",   1040, 11, 10 },
    { "°“Ώ",   1044, 11, 24 },
    { "‰i³",   1046,  4, 14 },   
    { "“Vμ",   1053,  1, 11 },
    { "N•½",   1058,  8, 29 },
    { "΅—ο",   1065,  8,  2 },
    { "‰„‹v",   1069,  4, 13 },
    { "³•Ϋ",   1074,  8, 23 },
    { "³—ο",   1077, 11, 17 },
    { "‰i•Ϋ",   1081,  2, 10 },
    { "‰“Ώ",   1084,  2,  7 },
    { "°΅",   1087,  4,  7 },
    { "‰Γ•Ϋ",   1094, 12, 15 },
    { "‰i’·",   1096, 12, 17 },
    { "³“Ώ",   1097, 11, 21 },
    { "Na",   1099,  8, 28 },
    { "’·΅",   1104,  2, 10 },
    { "‰Γ³",   1106,  4,  9 },
    { "“Vm",   1108,  8,  3 },
    { "“V‰i",   1110,  7, 13 },
    { "‰i‹v",   1113,  7, 13 },
    { "³‰i",   1118,  4,  3 },
    { "•Ϋΐ",   1120,  4, 10 },
    { "“V΅",   1124,  4,  3 },
    { "‘ε΅",   1126,  1, 22 },
    { "“V³",   1131,  1, 29 },
    { "’·³",   1132,  8, 11 },
    { "•Ϋ‰„",   1135,  4, 27 },
    { "‰i΅",   1141,  7, 10 },
    { "N΅",   1142,  4, 28 },
    { "“V—{",   1144,  2, 23 },
    { "‹vΐ",   1145,  7, 22 },
    { "m•½",   1151,  1, 26 },
    { "‹vυ",   1154, 10, 28 },
    { "•Ϋ³",   1156,  4, 27 },
    { "•½΅",   1159,  4, 20 },
    { "‰i—ο",   1160,  1, 10 },
    { "‰•Ϋ",   1161,  9,  4 },
    { "’·°",   1163,  3, 29 },
    { "‰i–",   1165,  6,  5 },
    { "mΐ",   1166,  8, 27 },
    { "‰Γ‰",   1169,  4,  8 },
    { "³ΐ",   1171,  4, 21 },
    { "ΐ³",   1175,  7, 28 },
    { "΅³",   1177,  8,  4 },
    { "—{a",   1181,  7, 14 },
    { "υ‰i",   1182,  5, 27 },
    { "³—ο",   1184,  4, 16 },
    { "•¶΅",   1185,  8, 14 },
    { "‹v",   1190,  4, 11 },
    { "³΅",   1199,  4, 27 },
    { "m",   1201,  2, 13 },
    { "³‹v",   1204,  2, 20 },
    { "‰i",   1206,  4, 27 },
    { "³³",   1207, 10, 25 },
    { "—ο",   1211,  3,  9 },
    { "•Ϋ",   1213, 12,  6 },
    { "³‹v",   1219,  4, 12 },
    { "’ε‰",   1222,  4, 13 },
    { "³m",   1224, 11, 20 },
    { "‰Γ\",   1225,  4, 20 },
    { "ΐ’ε",   1227, 12, 10 },
    { "°μ",   1229,  3,  5 },
    { "’ε‰i",   1232,  4,  2 },
    { "“V•",   1233,  4, 15 },
    { "•¶—ο",   1234, 11,  5 },
    { "‰Γ’υ",   1235,  9, 19 },
    { "—οm",   1238, 11, 23 },
    { "‰„‰",   1239,  2,  7 },
    { "m΅",   1240,  7, 16 },
    { "°³",   1243,  2, 26 },
    { "•σ΅",   1247,  2, 28 },
    { "’·",   1249,  3, 18 },
    { "N³",   1256, 10,  5 },
    { "³‰Γ",   1257,  3, 14 },
    { "³³",   1259,  3, 26 },
    { "•¶‰",   1260,  4, 13 },
    { "O’·",   1261,  2, 20 },
    { "•¶‰i",   1264,  2, 28 },
    { "΅",   1275,  4, 25 },
    { "Oΐ",   1278,  2, 29 },
    { "³‰",   1288,  4, 28 },
    { "‰im",   1293,  8,  5 },
    { "³ΐ",   1299,  4, 25 },
    { "£³",   1302, 11, 21 },
    { "‰Γ³",   1303,  8,  5 },
    { "“Ώ΅",   1306, 12, 14 },
    { "‰„c",   1308, 10,  9 },
    { "‰’·",   1311,  4, 28 },
    { "³a",   1312,  3, 20 },
    { "•¶•Ϋ",   1317,  2,  3 },
    { "³‰",   1319,  4, 28 },
    { "³‹",   1321,  2, 23 },
    { "³’†",   1324, 12,  9 },
    { "‰Γ—ο",   1326,  4, 26 },
    { "³“Ώ",   1329,  8, 29 },
   /* “μ–k’©•ª—τ */
    { "–Ύ“Ώ",   1390,  3, 26 }, /* 1393”N‰[105“ϊA“μ–k’©‡κ */
    { "‰‰i",   1394,  7,  5 },
    { "³’·",   1428,  4, 27 },
    { "‰i‹",   1429,  9,  5 },
    { "‰Γ‹g",   1441,  2, 17 },
    { "•¶ΐ",   1444,  2,  5 },
    { "•σ“Ώ",   1449,  7, 28 },
    { "‹“Ώ",   1452,  7, 25 },
    { "N³",   1455,  7, 25 },
    { "’·\",   1457,  9, 28 },
    { "°³",   1460, 12, 21 },
    { "•¶³",   1466,  2, 28 },
    { "‰m",   1467,  3,  5 },
    { "•¶–Ύ",   1469,  4, 28 },
    { "’·‹",   1487,  7, 20 },
    { "‰„“Ώ",   1489,  8, 21 },
    { "–Ύ‰",   1492,  7, 19 },
    { "•¶‹T",   1501,  2, 29 },
    { "‰i³",   1504,  2, 30 },
    { "‘ε‰i",   1521,  8, 23 },
    { "‹\",   1528,  8, 20 },
    { "“V•¶",   1532,  7, 29 },
    { "O΅",   1555, 10, 23 },
    { "‰i\",   1558,  2, 28 },
    { "³‹T",   1570,  4, 23 },
    { "“V³",   1573,  7, 28 },
    { "•¶\",   1592, 12,  8 },
    { "c’·",   1596, 10, 27 },
    { "³a",   1615,  7, 13 },
    { "°‰i",   1624,  2, 30 },
    { "³•Ϋ",   1644, 12, 16 },
    { "cΐ",   1648,  2, 15 },
    { "³δ",   1652,  9, 18 },
    { "–Ύ—ο",   1655,  4, 13 },
    { "δέ΅",   1658,  7, 23 },
    { "°•¶",   1661,  4, 25 },
    { "‰„›",   1673,  9, 21 },
    { "“Va",   1681,  9, 29 },
    { "’ε‹",   1684,  2, 21 },
    { "³\",   1688,  9, 30 },
    { "›‰i",   1704,  3, 13 },
    { "³“Ώ",   1711,  4, 25 },
    { "‹•Ϋ",   1716,  6, 22 },
    { "³•¶",   1736,  4, 28 },
    { "°•Ϋ",   1741,  2, 27 },
    { "‰„‹",   1744,  2, 21 },
    { "°‰„",   1748,  7, 12 },
    { "›—ο",   1751, 10, 27 },
    { "–Ύa",   1764,  6,  2 },
    { "ΐ‰i",   1772, 11, 16 },
    { "“V–Ύ",   1781,  4,  2 },
    { "°­",   1789,  1, 25 },
    { "‹a",   1801,  2,  5 },
    { "•¶‰»",   1804,  2, 11 },
    { "•¶­",   1818,  4, 22 },
    { "“V•Ϋ",   1830, 12, 10 },
    { "O‰»",   1844, 12,  2 },
    { "‰Γ‰i",   1848,  2, 28 },
    { "ΐ­",   1854, 11, 27 },
    { "δέ‰„",   1860,  3, 18 },
    { "•¶‹v",   1861,  2, 19 },
    { "³΅",   1864,  2, 20 },
    { "cδ",   1865,  4,  7 },
    { "–Ύ΅",   1868,  9,  8 },
    { "‘ε³",   1912,  7, 30 },
    { "Ίa",   1926, 12, 25 },
    { "•½¬",   1989,  1,  8 },
    { NULL,     9999, 12, 31 }
};
#define gengo(i)            gengo_tbl[i].name
#define gengo_start_yy(i)   gengo_tbl[i].start_yy
#define gengo_start_mm(i)   gengo_tbl[i].start_mm
#define gengo_start_dd(i)   gengo_tbl[i].start_dd

static struct GENGO_TBL southGengo[] =
{  /* s“μ’©t */
    { "³O",   1331,  8,  9 },
    { "•",   1334,  1, 29 },
    { "‰„³",   1336,  2, 29 },
    { "‹»‘",   1340,  4, 28 },
    { "³•½",   1346, 12,  8 },
    { "“Ώ",   1370,  7, 24 },
    { "•¶’†",   1372, 10,  4 },
    { "“Vφ",   1375,  5, 27 },
    { "Oa",   1381,  2, 10 },
    { "³’†",   1384,  4, 28 },
   /* ³’†9(1392)”N‰[105“ϊA–k’©Eγ¬Ό“Vc‚ΙχΚ(“μ–k’©‡κ) */
    { NULL,     1394,  7,  5 }
};

static struct GENGO_TBL northGengo[] =
{  /* s–k’©t */
    { "³“Ώ",   1329,  8, 29 },
    { "³c",   1332,  4, 28 },
   /* ³c2(1333)”N525“ϊ γ‘ην“Vc‚ΙχΚA³†”p~ */
    { "•",   1334,  1, 29 },
   /* •3(1336)”N815“ϊ –k’©Eυ–Ύ“Vc‘¦Κ */
    { "—ο‰",   1338,  8, 28 },
    { "N‰i",   1342,  4, 27 },
    { "’εa",   1345, 10, 21 },
    { "Ο‰",   1350,  2, 27 },
    { "•¶a",   1352,  9, 27 },
    { "‰„•¶",   1356,  3, 28 },
    { "Nΐ",   1361,  3, 29 },
    { "’ε΅",   1362,  9, 23 },
    { "‰ΐ",   1368,  2, 18 },
    { "‰ia",   1375,  2, 27 },
    { "N—ο",   1379,  3, 22 },
    { "‰i“Ώ",   1381,  2, 24 },
    { "“Ώ",   1384,  2, 27 },
    { "‰Γc",   1387,  8, 23 },
    { "N‰",   1389,  2,  9 },
    { "–Ύ“Ώ",   1390,  3, 26 },
    { NULL,     1394,  7,  5 }
};

#define sGengo(i)            southGengo[i].name
#define sGengo_start_yy(i)   southGengo[i].start_yy
#define sGengo_start_mm(i)   southGengo[i].start_mm
#define sGengo_start_dd(i)   southGengo[i].start_dd

#define nGengo(i)            northGengo[i].name
#define nGengo_start_yy(i)   northGengo[i].start_yy
#define nGengo_start_mm(i)   northGengo[i].start_mm
#define nGengo_start_dd(i)   northGengo[i].start_dd
#endif

static char	*hoshi[]     = { "‹γ‡", "”ª”’", "µΤ", "Z”’", "ά‰©",
                                 "l—Ξ", "O•Ι", "“ρ•", "κ”’" };
static char	*kan[]       = { "b", "‰³",
                                 "•Έ", "’",
                                 "•θ", "Θ",
                                 "M", "h",
                                 "p", "α΅" };
static char	*kan_yomi[]  = { "‚«‚Μ‚¦",   "‚«‚Μ‚Ζ",
                                 "‚Π‚Μ‚¦",   "‚Π‚Μ‚Ζ",
                                 "‚Β‚Ώ‚Μ‚¦", "‚Β‚Ώ‚Μ‚Ζ",
                                 "‚©‚Μ‚¦",   "‚©‚Μ‚Ζ",
                                 "‚έ‚Γ‚Μ‚¦", "‚έ‚Γ‚Μ‚Ζ" };
static char	*shi[]       = { "q", "‰N", "“Π", "‰K", "’C", "–¤",
                                 "ί", "–Ά", "\", "“Ρ", "ϊ", "ε" };
static char	*shi_yomi[]  = { "‚Λ", "‚¤‚µ", "‚Ζ‚η", "‚¤", "‚½‚Β", "‚έ",
                                 "‚¤‚ά", "‚Π‚Β‚¶", "‚³‚ι", "‚Ζ‚θ", "‚Ά‚Κ",
                                 "‚ξ" };
static char	*yoh[]       = { "“ϊ", "", "‰Ξ", "…",
                                 "–Ψ", "‹ΰ", "“y" };
static char	*tsuki[]     = { "–r", "”@",   "–ν¶", "‰K",
                                 "H", "…–³", "•¶", "—t",
                                 "’·", "_–³", "‘", "t‘–" };
static char	*shuku[]     = { "p", "΄","‚Δ‚Ά","–[", "S", "”φ", "–¥",
                                 "“l", "‹", "—", "‹•", "λ", "Ί", "•Η",
                                 "υ", "K", "έ", "γ", "•L", "ζX", "Q",
                                 "δ", "‹S", "–φ", "―", "’£", "—ƒ", "ηf"};
static char	*choku[]     = { "‚½‚Β",   "‚Μ‚Ό‚­", "‚έ‚Β",   "‚½‚Ά‚η",
                                 "‚³‚Ύ‚ρ", "‚Ζ‚ι",   "‚β‚Τ‚ι", "‚ ‚β‚Τ",
                                 "‚Θ‚ι",   "‚¨‚³‚ρ", "‚Π‚η‚­", "‚Ζ‚Γ"   };

char    *wareki2( int d, int m, int y, int u );

char	*
wareki( int d, int m, int y, int u )
{
    static char	buf[32];
    int		i;

    if ( (y >= 1331) && (y <= 1392) ) {
        long    g      = absoluteFromGregorian( d,  m,    y );
        long    gStart = absoluteFromGregorian( 9,  8, 1331 );
        long    gEnd   = absoluteFromGregorian( 5, 10, 1392 );

        if ( (g >= gStart) && (g <= gEnd) ) {
            return ( wareki2( d, m, y, u ) );
        }
    }

    for ( i = 0; gengo(i); i++ ) {
        if ( y < gengo_start_yy(i) )
            break;
        if ( y == gengo_start_yy(i) ) {
            if ( m < gengo_start_mm(i) )
                break;
            else if ( m == gengo_start_mm(i) ) {
                if ( d < gengo_start_dd(i) )
                    break;
            }
        }
    }
    if ( --i < 0 ) {
        i++;
        y--;
    }
    y -= gengo_start_yy(i) - 1;

    if ( u )
        sprintf( buf, "%s%2d”N‰[%2d(%s)%2d“ϊ",
                 gengo(i), y, m, tsuki[(m + 11) % 12], d );
    else
        sprintf( buf, "%s%2d”N%2d(%s)%2d“ϊ",
                 gengo(i), y, m, tsuki[(m + 11) % 12], d );
    return ( buf );
}


char	*
tsupodate( int day, int month, int year )
{
    static char	buf[20];

    if ( ( month == 1 ) && ( day == 1 ) )
        sprintf( buf, "%4d”N    ³“ϊ", year );
    else if ( ( month == 12 ) && ( day == 31 ) )
        sprintf( buf, "%4d”N  ‘εA“ϊ", year );
    else if ( day == 1 )
        sprintf( buf, "%4d”N%sρ“ϊ", year, tsuki[month-1] );
    else if ( day == lastDayOfGregorianMonth( month, year ) )
        sprintf( buf, "%4d”N%sA“ϊ", year, tsuki[month-1] );
    else
        sprintf( buf, "%4d”N%s%2d“ϊ", year, tsuki[month-1], day );
    if ( ( month != 6 ) && ( month != 10 ) ) {
        register char	*p = &buf[15];
        register int	i;

        for ( i = 0; i < 16; i++, p-- )
            *( p + 2 ) = *p;
        buf[0] = ' ';
        buf[1] = ' ';
    }

    return ( buf );
}

/*
 * ‹γ―Zo—
 *	‹γ―‚Ν‚ ‚ιν‚Μ“ϊ‚π‹«–Ϊ‚Ι‚µ‚Δ‘JΪ•ϋό‚ª‹t“]‚·‚ι
 *	—α‚¦‚Ξ 1990”N1224“ϊ ‚ά‚Ε‚Ν ‹γ‡¨”ª”’¨µΤ¨c¨κ”’ ‚Μ‡‚Ε‘JΪ‚·
 *	‚ι‚ªA 1990”N1225“ϊ ‚©‚η‚Ν κ”’¨“ρ•¨O•Ι¨c¨‹γ‡ ‚Μ‡‚Ε‘JΪ‚·
 *	‚ιB‚Ν‚Ά‚Β•Ο‚ν‚ι‚Μ‚©A•s•Χ‹­‚Μ‚½‚ί•s–Ύ‚Ε‚ ‚ιB‘JΪ•ϋό‚Μ‹t“]“ϊ‚ΜZ
 *	o‚Ν΅γ‚Μ‰Ϋ‘θ‚Ε‚ ‚ι                                  1991.02.17
 *  -----------------------------------------------------------------------
 *      —ο‚π©‚Δ‚έ‚ι‚Ζ180“ϊibq3‰ρj–‚Ι•Ο‚ν‚ι‚ζ‚¤‚Ε‚ ‚ι
 *      (‚ά‚½‚ΝA‰Δ‚ΙΕ‚ΰ‹ί‚Ά‹γ‡A“~‚ΙΕ‚ΰ‹ί‚Άκ”’‚Ε•Ο‚ν‚ι??)
 *                                                            1991.02.24
 *  -----------------------------------------------------------------------
 *  ‹γ―‚ΜZo•ϋ–@‚Ν—¬”h‚Ι‚ζ‚θA‚Ά‚λ‚Ά‚λ‚Θ‚ΰ‚Μ‚ª‘¶έ‚·‚ι‚±‚Ζ‚ª”»–ΎB
 *  ‹γ―‚Ν’†‘‚ΕlΔ‚³‚κ‚½‚ΰ‚Μ‚Ύ‚ªA“ϊ–{‚Ι“ό‚Α‚Δ‚­‚ι‚Ζ‚«‚ΙA“ϊ–{“Ζ©‚ΜƒAƒ
 *  ƒ“ƒW‚ª‰Α‚¦‚η‚κA‚³‚η‚Ι‚Ά‚λ‚Ά‚λ‚Θ—¬”h‚Ι•ª‚©‚κ‚½‚±‚Ζ‚ª΄φB
 *
 *    E—z“Ω
 *       κ”’¨“ρ•¨O•Ι¨c¨‹γ‡ ‚Μ‡Κ‚Ε‘JΪ
 *    E‰A“Ω
 *       ‹γ‡¨”ª”’¨µΤ¨c¨κ”’ ‚Μ‡Κ‚Ε‘JΪ
 *
 *   —z“Ω‚Ζ‰A“Ω‚ΜΨ‘Φ‚¦ϊ
 *    Θ‰Ί‚Μ‚ζ‚¤‚ΙA‚Ά‚λ‚Ά‚λ‚Θ—¬”h‚ª‘¶έ‚·‚ι
 *     * ‰ΔA“~
 *     * “~‚ΙΕ‚ΰ‹ί‚Άbq‚Μ“ϊ‚πκ”’‚Ζ‚µ‚Δ—z“Ω‚πn‚ίA‰Δ‚Ι
 *       Ε‚ΰ‹ί‚Άbq‚Μ“ϊ‚π‹γ‡‚Ζ‚µ‚Δ‰A“Ω‚πn‚ί‚ι
 *       (“~E‰Δ‚Μ’Όγ‚Ε‚Ν‚Θ‚­A’Ό‘O‚Μκ‡‚ΰ‚ ‚θ“Ύ‚ι)
 *     * “~‚ΙΕ‚ΰ‹ί‚Άbq‚Μ“ϊ‚πκ”’‚Ζ‚µ‚Δ—z“Ω‚πn‚ίA‰Δ‚Ι
 *       Ε‚ΰ‹ί‚Άbq‚Μ“ϊ‚π‹γ‡‚Ζ‚µ‚Δ‰A“Ω‚πn‚ί‚ιB‚½‚Ύ‚µA“~
 *       E‰Δ‚Μ‘Oγ1“ϊΘ“ΰ‚Ιbί‚ª‚ ‚ικ‡A‚Β‚ά‚θ“~E
 *       ‰Δ‚ªα΅–¤EbίE‰³–Ά‚Μ‚Ά‚Γ‚κ‚©‚Ε‚ ‚ικ‡‚ΝA‚»‚Μb
 *       ί‚Μ“ϊ‚ΕΨ‚θ‘Φ‚¦‚ι‚ªA“~‘Oγ‚Μbί“ϊ‚ΝµΤ‚©‚ηA‰Δ
 *       ‘Oγ‚Μbί“ϊ‚ΝO•Ι‚©‚ηn‚ί‚ι(‚±‚κ‚π‰[‚Ζ‚Ά‚¤)B
 *     * “~’Όγ‚Μ‹γ‡A‰Δ’Όγ‚Μκ”’
 *     * “~’Ό‘O‚Μ‹γ‡A‰Δ’Ό‘O‚Μκ”’
 *     * ‰ΔE“~‚Ε‚Ν‚Θ‚­A¬‹E¬¦‚Ε‰A“ΩE—z“Ω‚πΨ‚θ‘Φ‚¦‚ι
 *     * 60”N‚²‚Ζ‚ΙA‰A“ΩE—z“Ω‚Μ‡‚π‹t“]‚³‚Ή‚ι(u‰Δ‹ί•Σ‚Ε—z
 *       “ΩJnA“~‹ί•Σ‚Ε‰A“ΩJnv‚Ζ‚Ά‚¤ƒpƒ^[ƒ“‚Ζu‰Δ‹ί•Σ
 *       ‚Ε‰A“ΩJnA“~‹ί•Σ‚Ε—z“ΩJnv‚Ζ‚Ά‚¤ƒpƒ^[ƒ“‚π60”N‚²
 *       ‚Ζ‚ΙΨ‘Φ‚¦‚ι)
 *     * —¬”h‚Ι‚ζ‚Α‚Δ‚Νu‰A“Ωv‚Μ‚έ‚Εu—z“Ωv‚Ν‘¶έ‚µ‚Θ‚Ά
 *     * ‚»‚Μ‘ΌA‚Ά‚λ‚Ά‚λ
 *
 *   –{ƒvƒƒOƒ‰ƒ€‚Ε‚ΝAΘ‰Ί‚ΜZo•ϋ–@‚πΜ—p‚·‚ιB
 *     —z“ΩJn: “~‚ΙΕ‚ΰ‹ί‚Άbq(κ”’)
 *     ‰A“ΩJn: —z“ΩJn“ϊ‚Μ180“ϊγ(‹γ‡)
 *
 *     [Θ‘OΜ—p‚µ‚Δ‚Ά‚½Zo•ϋ–@]
 *       —z“ΩJn: 1990”N1225“ϊ‚¨‚ζ‚ΡA‚»‚Μ360‚Μ”{”“ϊ‘OEγ
 *       ‰A“ΩJn: —z“ΩJn“ϊ‚Μ180“ϊ‘OEγ
 *         ‚±‚Μ•ϋ–@‚Ύ‚ΖA‰Δ‚β“~‚Μ“ϊ‚π‹‚ί‚ι‚±‚Ζ‚Θ‚­A‘S‚Δ‚Μ“ϊ‚Μ
 *         ‹γ―‚πZo‰Β”\‚Ε‚ ‚ι(³m‚Ι180“ϊ‚²‚Ζ‚Ι—z“ΩE‰A“Ω‚ª“ό‚κ‘Φ
 *         ‚ν‚ι)B‚µ‚©‚µA–”NA365 - 180 * 2 = 5“ϊ(‚ά‚½‚Ν6“ϊ)‚Γ‚Β—z
 *         “ΩJn“ϊ‚ª‚Έ‚κ‚Δ‚Ά‚­‚Ζ‚Ά‚¤‡“_‚ª‚ ‚ιB
 *
 *   Ql
 *    Yoshifumi Mori(X‰ΐj)‚Μ today ‚Ε‚ΝA
 *      “~‚Μ±x‚ª
 *        bq ` p’C ‚Μ‚Ζ‚«
 *          —z“ΩJn: “~‚Μ’Ό‘O‚Μκ”’‚Μ“ϊ‚©‚η
 *        •Έ\ ` α΅ε ‚Μ‚Ζ‚«
 *          —z“ΩJn: “~‚Μ’Όγ‚Μκ”’‚Μ“ϊ‚©‚η
 *        ‚»‚κΘO (α΅–¤,bί,‰³–Ά) ‚Μ‚Ζ‚«
 *          —z“ΩJn: “~‚Ι‹ί‚ΆµΤ‚Μ“ϊ‚©‚η
 *      ‰Δ‚Μ±x‚ª
 *        bq ` p’C ‚Μ‚Ζ‚«
 *          ‰A“ΩJn: ‰Δ‚Μ’Ό‘O‚Μ‹γ‡‚Μ“ϊ‚©‚η
 *        •Έ\ ` α΅ε ‚Μ‚Ζ‚«
 *          ‰A“ΩJn: ‰Δ‚Μ’Όγ‚Μ‹γ‡‚Μ“ϊ‚©‚η
 *        ‚»‚κΘO (α΅–¤,bί,‰³–Ά) ‚Μ‚Ζ‚«
 *          ‰A“ΩJn: ‰Δ‚Ι‹ί‚ΆO•Ι‚Μ“ϊ‚©‚η
 *    ‚Ζ‚Ά‚¤Zo•ϋ–@‚πΜ—p‚µ‚Δ‚Ά‚ιB
 *
 *      —α: 2003”N 3 6“ϊ
 *        –{ƒvƒƒOƒ‰ƒ€ [Θ‘OΜ—p‚µ‚Δ‚Ά‚½Zo•ϋ–@]
 *           ƒOƒƒSƒƒI—ο    2003”N   3(March) 6“ϊ  –Ψ—j“ϊ  Lent
 *           a—ο ‘Ύ—z—π     •½¬15”N 3(–ν¶)  6“ϊ  ‹γ‡  p  ‚Ζ‚Γ  O—Χ–S
 *                ‘Ύ‰A‘Ύ—z—ο •½¬15”N 2(”@)  4“ϊ  ‘εΐ  [ε­  ’†’ª
 *                ±x       α΅–Ά(‚έ‚Γ‚Μ‚Ζ‚Π‚Β‚¶)‚Μ”N •θ“Π(‚Β‚Ώ‚Μ‚¦‚Ζ‚η)‚Μ“ϊ
 *                           ¬”Ζ“y
 *
 *        –{ƒvƒƒOƒ‰ƒ€ [V‚µ‚ΆZo•ϋ–@]
 *           ƒOƒƒSƒƒI—ο    2003”N   3(March) 6“ϊ  –Ψ—j“ϊ  Lent
 *           a—ο ‘Ύ—z—π     •½¬15”N 3(–ν¶)  6“ϊ  O•Ι  p  ‚Ζ‚Γ  O—Χ–S
 *                ‘Ύ‰A‘Ύ—z—ο •½¬15”N 2(”@)  4“ϊ  ‘εΐ  [ε­  ’†’ª
 *                ±x       α΅–Ά(‚έ‚Γ‚Μ‚Ζ‚Π‚Β‚¶)‚Μ”N •θ“Π(‚Β‚Ώ‚Μ‚¦‚Ζ‚η)‚Μ“ϊ
 *                           ¬”Ζ“y
 *
 *        X‚Μ today
 *           2003”N 3 6“ϊ (–Ψ—j“ϊ)
 *           ‹—ο     2003”N 2 4“ϊ (‘εΐ) [’†’ª] 
 *           ±x     α΅–Ά (‚έ‚Έ‚Μ‚Ζ‚Π‚Β‚¶) ‚Μ”NA•θ“Π (‚Β‚Ώ‚Μ‚¦‚Ζ‚η) ‚Μ“ϊ
 *           ‹γ―     O•Ι
 *           \“ρ’Ό   •Β
 *           “ρ\”ªh p
 *           [ε­ O—Χ–S κ—±–”{“ϊ ¬”Ζ“y ‹Aυ“ϊ “VΝ“ϊ
 */

char	*
kyusei( long j )    /* j: ƒ†ƒƒEƒX“ϊ */
{
    int     hday;

#if 0
    long    change;

    /* Θ‰Ί‚ΜvZ®‚ΝA1990”N1225“ϊ‚ª—z“ΩJn“ϊ‚Ε‚ ‚ιA‚Ζ‚Ά‚¤–ΐ(?)‚π
       ξ‚Ι‚µ‚Δ‚Ά‚ι */
    j -= 3;
    change = ( j + 17 ) % 360;

    /* 180“ϊ‚²‚Ζ‚Ι—z“ΩE‰A“Ω‚πJ‚θ•Τ‚· */
    hday   = ( change >= 180 ) ? ( j - 1 ) % 9      /* ‰A“Ω */
                               : ( 9 - j % 9 ) % 9; /* —z“Ω */
#else
    int     yy, mm, dd;
    long    midWinterDay, kouShiDay;
    int     kanShi;

    gregorianFromAbsolute( j, &dd, &mm, &yy );
    midWinterDay = getSekkiDate( 20, 12, yy );      /* “~ */

    /* “~‚Ι‹ί‚Άbq‚Μ“ϊ */
    do {
        kanShi = ( midWinterDay + 14 ) % 60;    /* “~‚Μ±x */
        if ( kanShi < 30 ) {
            /* “~‚Ι‹ί‚Άbq‚Μ“ϊ‚ΝA“~‚ζ‚θ‘O */
            kouShiDay = midWinterDay - kanShi;
        }
        else {
            /* “~‚Ι‹ί‚Άbq‚Μ“ϊ‚ΝA“~‚ζ‚θγ */
            kouShiDay = midWinterDay + (60 - kanShi);
        }
        if ( j < kouShiDay ) {
            yy--;
            midWinterDay = getSekkiDate( 20, 12, yy );  /* 1”N‘O‚Μ“~ */
        }
    } while ( j < kouShiDay );

    hday   = (j >= kouShiDay + 180)
                ? ( j - kouShiDay + 9 ) % 9    	        /* ‰A“Ω */
                : ( 9 - ( j - kouShiDay + 1 ) % 9) % 9; /* —z“Ω */
#endif

    return ( hoshi[ hday ] );	/* ‹γ― */
}

/*
 *	±x
 *
 * bq 0 ‰³‰N 1 •Έ“Π 2 ’‰K 3 •θ’C 4 Θ–¤ 5 Mί 6 h–Ά 7 p\ 8 α΅“Ρ 9
 * bϊ10 ‰³ε11 •Έq12 ’‰N13 •θ“Π14 Θ‰K15 M’C16 h–¤17 pί18 α΅–Ά19
 * b\20 ‰³“Ρ21 •Έϊ22 ’ε23 •θq24 Θ‰N25 M“Π26 h‰K27 p’C28 α΅–¤29
 * bί30 ‰³–Ά31 •Έ\32 ’“Ρ33 •θϊ34 Θε35 Mq36 h‰N37 p“Π38 α΅‰K39
 * b’C40 ‰³–¤41 •Έί42 ’–Ά43 •θ\44 Θ“Ρ45 Mϊ46 hε47 pq48 α΅‰N49
 * b“Π50 ‰³‰K51 •Έ’C52 ’–¤53 •θί54 Θ–Ά55 M\56 h“Ρ57 pϊ58 α΅ε59
 */
char	*
eto( long j, int year, int htmlMode )
{
    int         kyear, syear, kday, sday;
    static char buf[64];

    kyear = ( year - 4 ) % 10; syear = ( year - 4 ) % 12;
    kday  = ( j + 4 ) % 10;    sday  = ( j + 2 ) % 12;

    sprintf( buf, "%s%s(%s%s)‚Μ”N%s%s%s(%s%s)‚Μ“ϊ",
             kan[kyear],       shi[syear],
             kan_yomi[kyear],  shi_yomi[syear],
             htmlMode ? "<BR>" : " ",
             kan[kday],        shi[sday],
             kan_yomi[kday],   shi_yomi[sday] );

    return ( buf );
}

void
checkEto( long j, int htmlMode )
{
    long    ks;
    int     reset = 1;

    ks = ( j + 14 ) % 60;
    if ( ( ks >= 20 ) && ( ks <= 29 ) ) /* b\ ‚©‚η α΅–¤ */
        tprintf( htmlMode, &reset, "  \•ϋ•ι" );
    if ( ks == 29 )                     /* α΅–¤ */
        tprintf( htmlMode, &reset, "  “Vκ“Vγ" );
    if ( ( ks == 48 ) || ( ( ks >= 50 ) && ( ks <= 53 ) ) ||
         ( ks == 56 ) || ( ks == 57 ) || ( ks == 59 ) )
        /* pq, b“Π, ‰³‰K, •Έ’C, ’–¤, M\, h“Ρ, α΅ε */
        tprintf( htmlMode, &reset, "  ”ªκ" );
    if ( ( ks >=6 ) && ( ks <= 12 ) )   /* Mί ‚©‚η •Έq */
        tprintf( htmlMode, &reset, "  ‘ε”Ζ“y" );
    if ( ( ks >= 14 ) && ( ks <= 20 ) ) /* •θ“Π ‚©‚η b\ */
        tprintf( htmlMode, &reset, "  ¬”Ζ“y" );

    if ( reset == 1 ) {
        if ( htmlMode )
            fputs( "<BR>", stdout );
    }
}

/*
 *	µ—j
 */
char	*
weekday( long g )
{
    while ( g < 0 ) {
        g += 7;
    }

    return ( yoh[ g % 7 ] );	/* —j“ϊ */
}

/*
 *	“ω”ªh
 */
char	*
nijuhachishuku( long g )
{
	return ( shuku[ ( g - 4 ) % 28 ] );
}

/*
 *	\“ρ’Ό (’†’i)
 *		“ρ\lί‹C‚Μuίv‚Ζ‚»‚Μ‘O“ϊ (–{—‚Μuί•ªv) ‚Ε“―κ‚Μu’Όv‚π
 *		—p‚Ά‚ιΘO‚ΝA‡‚Ι‘JΪ‚µ‚Δ‚Ά‚­
 *		‚µ‚½‚ª‚Α‚ΔA”CΣ‚Μ“ϊ‚Μu’Όv‚π‹‚ί‚ι‚Ι‚Ν“ρ\lί‹C‚π‹‚ί‚ι•K—v
 *		‚ª‚ ‚ι
 *
 *	‚½‚Β	 0	‚Μ‚Ό‚­	 1	‚έ‚Β	 2	‚½‚Ά‚η	 3
 *	‚³‚Ύ‚ρ	 4	‚Ζ‚ι	 5	‚β‚Τ‚ι	 6	‚ ‚β‚Τ	 7
 *	‚Θ‚ι	 8	‚¨‚³‚ρ	 9	‚Π‚η‚­	10	‚Ζ‚Γ	11
 */

char	*
junichoku( long g, int dd, int mm, int yy, int *tsuki, long *fushi )
{
    static char	buf[80];
    long        g0 = 0, g2;
    int		si, sday;

    *fushi = g0 = setsuStartDay( mm, yy );	/* ί“ό‚θ‚Μ“ϊ */
    si = mm % 12;
    g2 = g0;
    do {
        sday = ( g2 + 2 ) % 12;
        g2++;
    } while ( sday != si );
    g2--;	/* g2: ί‚Ι‘Ξ‰‚·‚ιΕ‰‚Μu(‚½‚Β)v‚Μ“ϊ */
    if ( g >= g2 ) {
        strcpy( buf, choku[(g-g2)%12] );
        *tsuki = (mm - 1) % 12;
    }
    else if ( g >= g0 ) {
        strcpy( buf, choku[(12-((g2-g)%12))%12] );
        *tsuki = (mm - 1) % 12;
    }
    else {
        strcpy( buf, choku[(12-((g2-g-1)%12))%12] );
        *tsuki = (mm - 2) % 12;
    }

    return ( buf );
}


char    *
sanrinbou( long g, int tsuki )
{
    char    *p = NULL;
    char    *q = "O—Χ–S";

    switch ( tsuki ) {
    case  1 : /* ‹ 1 */
    case  4 : /* ‹ 4 */
    case  7 : /* ‹ 7 */
    case 10 : /* ‹10 */
        if ( ( g + 2 ) % 12 == 11 )	/* ε */
            p = q;
        break;
    case  2 : /* ‹ 2 */
    case  5 : /* ‹ 5 */
    case  8 : /* ‹ 8 */
    case 11 : /* ‹11 */
        if ( ( g + 2 ) % 12 == 2 )	/* “Π */
            p = q;
        break;
    case  3 : /* ‹ 3 */
    case  6 : /* ‹ 6 */
    case  9 : /* ‹ 9 */
    case  0 : /* ‹12 */
        if ( ( g + 2 ) % 12 == 6 )	/* ί */
            p = q;
        break;
    }

    return( p );
}

char    *
manbaibi( long g, int tsuki )
{
    char    *p = NULL;
    char    *q = "κ—±–”{“ϊ";
    int     eto = ( g + 2 ) % 12;

#if     0
    switch ( tsuki ) {
    case  1 : /* ‹ 1 */
	if ( (eto == 1) || (eto == 6) ) /* ‰N, ί */
	    p = q;
	break;
    case  2 : /* ‹ 2 */
	if ( (eto == 2) || (eto == 9) ) /* “Π, “Ρ */
	    p = q;
	break;
    case  3 : /* ‹ 3 */
	if ( (eto == 3) || (eto == 0) ) /* ‰K, q */
	    p = q;
	break;
    case  4 : /* ‹ 4 */
	if ( (eto == 4) || (eto == 3) ) /* ’C, ‰K */
	    p = q;
	break;
    case  5 : /* ‹ 5 */
	if ( (eto == 5) || (eto == 6) ) /* –¤, ί */
	    p = q;
	break;
    case  6 : /* ‹ 6 */
	if ( (eto == 6) || (eto == 9) ) /* ί, “Ρ */
	    p = q;
	break;
    case  7 : /* ‹ 7 */
	if ( (eto == 7) || (eto == 0) ) /* –Ά, q */
	    p = q;
	break;
    case  8 : /* ‹ 8 */
	if ( (eto == 8) || (eto == 3) ) /* \, ‰K */
	    p = q;
	break;
    case  9 : /* ‹ 9 */
	if ( (eto == 9) || (eto == 6) ) /* “Ρ, ί */
	    p = q;
	break;
    case 10 : /* ‹10 */
	if ( (eto == 10) || (eto == 9) )/* ϊ, “Ρ */
	    p = q;
        break;
    case 11 : /* ‹11 */
	if ( (eto == 11) || (eto == 0) )/* ε, q */
	    p = q;
        break;
    case  0 : /* ‹12 */
	if ( (eto == 0) || (eto == 3) ) /* q, ‰K */
	    p = q;
        break;
    }
#else
    if ( (eto == tsuki) || (eto == (((tsuki + 1) * 3) % 12)) )
	p = q;
#endif

    return( p );
}


char    *
kurobi( long g, int tsuki )
{
    char    *p = NULL;
    char    *q = "σ€“ϊ";

#if     0
    switch ( tsuki ) {
    case  1 : /* ‹ 1 */
        if ( ( g + 2 ) % 12 == 10 )	/* ϊ */
            p = q;
        break;
    case  2 : /* ‹ 2 */
        if ( ( g + 2 ) % 12 == 4 )	/* ’C */
            p = q;
        break;
    case  3 : /* ‹ 3 */
        if ( ( g + 2 ) % 12 == 11 )	/* ε */
            p = q;
        break;
    case  4 : /* ‹ 4 */
        if ( ( g + 2 ) % 12 == 5 )	/* –¤ */
            p = q;
        break;
    case  5 : /* ‹ 5 */
        if ( ( g + 2 ) % 12 == 0 )	/* q */
            p = q;
        break;
    case  6 : /* ‹ 6 */
        if ( ( g + 2 ) % 12 == 6 )	/* ί */
            p = q;
        break;
    case  7 : /* ‹ 7 */
        if ( ( g + 2 ) % 12 == 1 )	/* ‰N */
            p = q;
        break;
    case  8 : /* ‹ 8 */
        if ( ( g + 2 ) % 12 == 7 )	/* –Ά */
            p = q;
        break;
    case  9 : /* ‹ 9 */
        if ( ( g + 2 ) % 12 == 2 )	/* “Π */
            p = q;
        break;
    case 10 : /* ‹10 */
        if ( ( g + 2 ) % 12 == 8 )	/* \ */
            p = q;
        break;
    case 11 : /* ‹11 */
        if ( ( g + 2 ) % 12 == 3 )	/* ‰K */
            p = q;
        break;
    case  0 : /* ‹12 */
        if ( ( g + 2 ) % 12 == 9 )	/* “Ρ */
            p = q;
        break;
    }
#else
    if ( (tsuki % 2) == 1 ) {
        if ( (( g + 2 ) % 12) == (((tsuki / 2) + 10) % 12) )
            p = q;
    }
    else {
	if ( tsuki == 0 )
	    tsuki = 12;
        if ( (( g + 2 ) % 12) == (((tsuki / 2) +  3) % 12) )
            p = q;
    }
#endif

    return( p );
}


char    *
jisshinichi( long g, int tsuki )
{
    char    *p = NULL;
    char    *q = "\€“ϊ";

    switch ( tsuki ) {
    case  1 : /* ‹ 1 */
    case  4 : /* ‹ 4 */
    case  7 : /* ‹ 7 */
    case 10 : /* ‹10 */
        if ( ( g + 2 ) % 12 == 9 )	/* “Ρ */
            p = q;
        break;
    case  2 : /* ‹ 2 */
    case  5 : /* ‹ 5 */
    case  8 : /* ‹ 8 */
    case 11 : /* ‹11 */
        if ( ( g + 2 ) % 12 == 5 )	/* –¤ */
            p = q;
        break;
    case  3 : /* ‹ 3 */
    case  6 : /* ‹ 6 */
    case  9 : /* ‹ 9 */
    case  0 : /* ‹12 */
        if ( ( g + 2 ) % 12 == 1 )	/* ‰N */
            p = q;
        break;
    }

    return( p );
}


char    *
kikonichi( long g, int tsuki )
{
    char    *p = NULL;
    char    *q = "‹Aυ“ϊ";

    switch ( tsuki ) {
    case  1 : /* ‹ 1 */
    case  4 : /* ‹ 4 */
    case  7 : /* ‹ 7 */
    case 10 : /* ‹10 */
        if ( ( g + 2 ) % 12 == 1 )	/* ‰N */
            p = q;
        break;
    case  2 : /* ‹ 2 */
    case  5 : /* ‹ 5 */
    case  8 : /* ‹ 8 */
    case 11 : /* ‹11 */
        if ( ( g + 2 ) % 12 == 2 )	/* “Π */
            p = q;
        break;
    case  3 : /* ‹ 3 */
    case  6 : /* ‹ 6 */
    case  9 : /* ‹ 9 */
    case  0 : /* ‹12 */
        if ( ( g + 2 ) % 12 == 0 )	/* q */
            p = q;
        break;
    }

    return( p );
}


char    *
fukunichi( long g, int tsuki )
{
    char    *p = NULL;
    char    *q = "•“ϊ";
    int     kan = ( g + 4 ) % 10;

    switch ( tsuki ) {
    case  1 : /* ‹ 1 */
    case  7 : /* ‹ 7 */
        if ( (kan == 0) || (kan == 6) )	/* b, M */
            p = q;
        break;
    case  2 : /* ‹ 2 */
    case  8 : /* ‹ 8 */
        if ( (kan == 1) || (kan == 7) )	/* ‰³, h */
            p = q;
	break;
    case  3 : /* ‹ 3 */
    case  6 : /* ‹ 6 */
    case  9 : /* ‹ 9 */
    case  0 : /* ‹12 */
        if ( (kan == 4) || (kan == 5) )	/* •θ, Θ */
            p = q;
	break;
    case  4 : /* ‹ 4 */
    case 10 : /* ‹10 */
        if ( (kan == 2) || (kan == 8) )	/* •Έ, p */
            p = q;
        break;
    case  5 : /* ‹ 5 */
    case 11 : /* ‹11 */
        if ( (kan == 3) || (kan == 9) )	/* ’, α΅ */
            p = q;
        break;
    }

    return( p );
}


char    *
chiImiNichi( long g, int tsuki )
{
    char    *p = NULL;
    char    *q = "υ“ϊ";

#if     0
    switch ( tsuki ) {
    case  1 : /* ‹ 1 */
        if ( ( g + 2 ) % 12 == 1 )	/* ‰N */
            p = q;
        break;
    case  2 : /* ‹ 2 */
        if ( ( g + 2 ) % 12 == 7 )	/* –Ά */
            p = q;
        break;
    case  3 : /* ‹ 3 */
        if ( ( g + 2 ) % 12 == 2 )	/* “Π */
            p = q;
        break;
    case  4 : /* ‹ 4 */
        if ( ( g + 2 ) % 12 == 8 )	/* \ */
            p = q;
        break;
    case  5 : /* ‹ 5 */
        if ( ( g + 2 ) % 12 == 3 )	/* ‰K */
            p = q;
        break;
    case  6 : /* ‹ 6 */
        if ( ( g + 2 ) % 12 == 9 )	/* “Ρ */
            p = q;
        break;
    case  7 : /* ‹ 7 */
        if ( ( g + 2 ) % 12 == 4 )	/* ’C */
            p = q;
        break;
    case  8 : /* ‹ 8 */
        if ( ( g + 2 ) % 12 == 10 )	/* ϊ */
            p = q;
        break;
    case  9 : /* ‹ 9 */
        if ( ( g + 2 ) % 12 == 5 )	/* –¤ */
            p = q;
        break;
    case 10 : /* ‹10 */
        if ( ( g + 2 ) % 12 == 11 )	/* ε */
            p = q;
        break;
    case 11 : /* ‹11 */
        if ( ( g + 2 ) % 12 == 6 )	/* ί */
            p = q;
        break;
    case  0 : /* ‹12 */
        if ( ( g + 2 ) % 12 == 0 )	/* q */
            p = q;
        break;
    }
#else
    if ( (tsuki % 2) == 1 ) {
        if ( (( g + 2 ) % 12) == (((tsuki / 2) + 1) % 12) )
            p = q;
    }
    else {
	if ( tsuki == 0 )
	    tsuki = 12;
        if ( (( g + 2 ) % 12) == (((tsuki / 2) +  6) % 12) )
            p = q;
    }
#endif

    return( p );
}


char    *
tenKaNichi( long g, int tsuki )
{
    char    *p = NULL;
    char    *q = "“V‰Ξ“ϊ";

    switch ( tsuki ) {
    case  1 : /* ‹ 1 */
    case  5 : /* ‹ 5 */
    case  9 : /* ‹ 9 */
        if ( ( g + 2 ) % 12 == 0 )	/* q */
            p = q;
        break;
    case  2 : /* ‹ 2 */
    case  6 : /* ‹ 6 */
    case 10 : /* ‹10 */
        if ( ( g + 2 ) % 12 == 3 )	/* ‰K */
            p = q;
        break;
    case  3 : /* ‹ 3 */
    case  7 : /* ‹ 7 */
    case 11 : /* ‹11 */
        if ( ( g + 2 ) % 12 == 6 )	/* ί */
            p = q;
        break;
    case  4 : /* ‹ 4 */
    case  8 : /* ‹ 8 */
    case  0 : /* ‹12 */
        if ( ( g + 2 ) % 12 == 9 )	/* “Ρ */
            p = q;
        break;
    }

    return( p );
}


char    *
jiKaNichi( long g, int tsuki )
{
    char    *p = NULL;
    char    *q = "’n‰Ξ“ϊ";

    if ( (( g + 2 ) % 12) == ((tsuki + 4) % 12) )
	p = q;

    return( p );
}


char    *
ouMouNichi( long g, int tsuki, long fushi )
{
    char    *p = NULL;
    char    *q = "‰–S“ϊ";

    switch ( tsuki ) {
    case  1 : /* ‹ 1 */
        if ( g == fushi + 6 )
            p = q;
        break;
    case  2 : /* ‹ 2 */
        if ( g == fushi + 13 )
            p = q;
        break;
    case  3 : /* ‹ 3 */
        if ( g == fushi + 20 )
            p = q;
        break;
    case  4 : /* ‹ 4 */
        if ( g == fushi + 7 )
            p = q;
        break;
    case  5 : /* ‹ 5 */
        if ( g == fushi + 15 )
            p = q;
        break;
    case  6 : /* ‹ 6 */
        if ( g == fushi + 23 )
            p = q;
        break;
    case  7 : /* ‹ 7 */
        if ( g == fushi + 8 )
            p = q;
        break;
    case  8 : /* ‹ 8 */
        if ( g == fushi + 17 )
            p = q;
        break;
    case  9 : /* ‹ 9 */
        if ( g == fushi + 26 )
            p = q;
        break;
    case 10 : /* ‹10 */
        if ( g == fushi + 9 )
            p = q;
        break;
    case 11 : /* ‹11 */
        if ( g == fushi + 19 )
            p = q;
        break;
    case  0 : /* ‹12 */
        if ( g == fushi + 29 )
            p = q;
        break;
    }

    return( p );
}


char    *
boSouNichi( long g, int tsuki )
{
    char    *p = NULL;
    char    *q = "•κ‘q“ϊ";
    int     eto = ( g + 2 ) % 12;

    switch ( tsuki ) {
    case  1 : /* ‹ 1 */
    case  2 : /* ‹ 2 */
	if ( (eto == 0) || (eto == 11) )/* q, ε */
	    p = q;
	break;
    case  3 : /* ‹ 3 */
    case  6 : /* ‹ 6 */
    case  9 : /* ‹ 9 */
    case  0 : /* ‹12 */
	if ( (eto == 5) || (eto == 6) )	/* –¤, ί */
	    p = q;
	break;
    case  4 : /* ‹ 4 */
    case  5 : /* ‹ 5 */
	if ( (eto == 2) || (eto == 3) )	/* “Π, ‰K */
            p = q;
        break;
    case  7 : /* ‹ 7 */
    case  8 : /* ‹ 8 */
	if ( (eto == 1) || (eto == 4) ||
	     (eto == 7) || (eto == 10))	/* ‰N, ’C, –Ά, ϊ */
            p = q;
        break;
    case 10 : /* ‹10 */
    case 11 : /* ‹11 */
	if ( (eto == 4) || (eto == 8) )	/* ’C, \ */
            p = q;
        break;
    }

    return( p );
}


char    *
tsukiTokuNichi( long g, int tsuki )
{
    char    *p = NULL;
    char    *q = "“Ώ“ϊ";
    int     kan = ( g + 4 ) % 10;

    switch ( tsuki ) {
    case  1 : /* ‹ 1 */
    case  5 : /* ‹ 5 */
    case  9 : /* ‹ 9 */
	if ( kan == 2 )     /* •Έ */
	    p = q;
	break;
    case  2 : /* ‹ 2 */
    case  6 : /* ‹ 6 */
    case 10 : /* ‹10 */
	if ( kan == 0 )     /* b */
	    p = q;
	break;
    case  3 : /* ‹ 3 */
    case  7 : /* ‹ 7 */
    case 11 : /* ‹11 */
	if ( kan == 8 )     /* p */
            p = q;
        break;
    case  4 : /* ‹ 4 */
    case  8 : /* ‹ 8 */
    case  0 : /* ‹12 */
	if ( kan == 8 )     /* Θ */
            p = q;
	break;
    }

    return( p );
}


char	*
japaneseMonthName( int mm )
{
    return ( tsuki[(mm + 11) % 12] );
}


/* “μ–k’©‘γ‚Μa—ο•\¦ */
char	*
wareki2( int d, int m, int y, int u )
{
    static char buf[48];
    int         i, j;
    int         nY = y, sY = y;

    for ( i = 0; nGengo(i); i++ ) {
        if ( nY < nGengo_start_yy(i) )
            break;
        if ( nY == nGengo_start_yy(i) ) {
            if ( m < nGengo_start_mm(i) )
                break;
            else if ( m == nGengo_start_mm(i) ) {
                if ( d < nGengo_start_dd(i) )
                    break;
            }
        }
    }
    if ( --i < 0 ) {
        i++;
        nY--;
    }
    nY -= nGengo_start_yy(i) - 1;

    for ( j = 0; sGengo(j); j++ ) {
        if ( sY < sGengo_start_yy(j) )
            break;
        if ( sY == sGengo_start_yy(j) ) {
            if ( m < sGengo_start_mm(j) )
                break;
            else if ( m == sGengo_start_mm(j) ) {
                if ( d < sGengo_start_dd(j) )
                    break;
            }
        }
    }
    if ( --j < 0 ) {
        j++;
        sY--;
    }
    sY -= sGengo_start_yy(j) - 1;

    if ( u )
	    sprintf( buf, "%s%2d”N(–k)/%s%2d”N(“μ) ‰[%2d(%s)%2d“ϊ",
                 nGengo(i), nY, sGengo(j), sY,
                 m, tsuki[(m + 11) % 12], d );
    else
	    sprintf( buf, "%s%2d”N(–k)/%s%2d”N(“μ) %2d(%s)%2d“ϊ",
                 nGengo(i), nY, sGengo(j), sY,
                 m, tsuki[(m + 11) % 12], d );
    return ( buf );
}

