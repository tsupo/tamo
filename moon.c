...
int yy, mm, dd, HH, MM, SS;
{
    ...
    printf(
  "<A HREF=\"moon.cgi?y=%04d&m=%02d&d=%02d&H=%02d&M=%02d&S=%02d\">去年</A>\n",
           yy - 1, mm, dd, HH, MM, SS );
    ...
    printf(
  "<A HREF=\"moon.cgi?y=%04d&m=%02d&d=%02d&H=%02d&M=%02d&S=%02d\">先月</A>\n",
           a3, a2, a1, HH, MM, SS );
    ...
    printf(
  "<A HREF=\"moon.cgi?y=%04d&m=%02d&d=%02d&H=%02d&M=%02d&S=%02d\">昨日</A>\n",
           a3, a2, a1, HH, MM, SS );

    printf("<A HREF=\"moon.cgi?y=0&m=0&d=0&H=0\">今日</A>\n");
    ...
    printf(
  "<A HREF=\"moon.cgi?y=%04d&m=%02d&d=%02d&H=%02d&M=%02d&S=%02d\">明日</A>\n",
           a3, a2, a1, HH, MM, SS );
    ...
    printf(
  "<A HREF=\"moon.cgi?y=%04d&m=%02d&d=%02d&H=%02d&M=%02d&S=%02d\">来月</A>\n",
           a3, a2, a1, HH, MM, SS );

    printf(
  "<A HREF=\"moon.cgi?y=%04d&m=%02d&d=%02d&H=%02d&M=%02d&S=%02d\">来年</A>\n",
           yy + 1, mm, dd, HH, MM, SS );
}
...
# endif
    ...
    printf( "世界協定時(UTC)%s %d年%02d月%02d日 %02d時%02d分%02d秒\n",
            (flag & 1) == 1 ? "</TD><TD>" : ":",
    ...
    if ( !strcmp( _tzname[lm->tm_isdst], "JST" ) ) {
        printf( "日本標準時(JST)%s %d年%02d月%02d日 %02d時%02d分%02d秒\n",
                (flag & 1) == 1 ? "</TD><TD COLSPAN=2>" : ":",
                lm->tm_year + 1900, lm->tm_mon + 1, lm->tm_mday,
                lm->tm_hour, lm->tm_min, lm->tm_sec );
    }
    else {
        printf( "ローカル時(%s)%s %d年%02d月%02d日 %02d時%02d分%02d秒\n",
                _tzname[0],
                (flag & 1) == 1 ? "</TD><TD>" : ":",
                lm->tm_year + 1900, lm->tm_mon + 1, lm->tm_mday,
                lm->tm_hour, lm->tm_min, lm->tm_sec );
    ...
    if ( (flag & 1) == 1 )
        printf( "</TD></TR>\n<TR><TD>" );
    printf( "地球-月の距離%s   %ldkm (地球赤道半径の%.1f倍)\n",
            (flag & 1) == 1 ? "</TD><TD>" : ":",
